...
class RegisterController extends FrontBaseController
{
	...
	public function register(Request $request)
	{
		...
		if ($gs->is_verification_email == 1) {
			...
			$msg =
				'Dear Customer,<br> We noticed that you need to verify your email address. <a href=' .
				url('user/register/verify/' . $token) .
				'>Simply click here to verify. </a>';
			...
			if ($gs->is_smtp == 1) {
				...
				$headers =
					'From: ' . $gs->from_name . '<' . $gs->from_email . '>';
				mail($to, $subject, $msg, $headers);
			}
			return response()->json(
				'We need to verify your email address. We have sent an email to ' .
				$to .
				' to verify your email address. Please click link in that email to continue.'
			);
		} else {
