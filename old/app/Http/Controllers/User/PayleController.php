...
class PayleController extends Controller
{
   ...
   public function store(Request $request){
    ...
    $item_amount = $subs->price;
                    ...
                    $input['method'] = 'Payle';
                    if($subs->subscription_type == 2){
                    $input['subscription_type'] = $subs->subscription_type;
                    $input['banner_type'] = $subs->banner_type;
                    }
    ...
    $udf1= url('/user/payle/payreturn');
                ...
                $url = 'https://trans.payleq8.com/apifee/requestfullamounts';
       ...
       public function payreturn(){
            ...
            if($code = '00' && $track_number != '' ){
                ...
                if('SHPEKT'.$requested->id == request('refnumber')){
                    $subscription = UserSubscription::find($requested->id); 
                    ...
                    $subscription->save();
                    return redirect('vendor/dashboard')->with('success','Vendor Account Activated Successfully');
                }
                else{
                     $subscription = UserSubscription::find($requested->id);    
                     $subscription->delete();
                     return redirect('vendor/dashboard')->with('failed','Vendor Account  Not Activated Successfully');
                }
            }
            else{   
                $subscription = UserSubscription::find($requested->id);    
