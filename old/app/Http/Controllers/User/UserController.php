...
class UserController extends Controller
{
    ...
    {
        $user = Auth::user();
        $subs = Subscription::where('subscription_type',2)->get();
        $package = $user->subscribes()->where('status',1)->orderBy('id','desc')->first();
        return view('user.banner.index',compact('user','subs','package'));
    }
    ...
    {
     $sub = UserSubscription::orderBy('id', 'DESC')->where('transaction_status','=',1)->where('subscription_type','=',2)->where('user_id','=',Auth::user()->id)->limit(1)->get();
   
          if($sub != "null"){
        $sub = $sub['0'];
         return view('user.banner',compact('sub'));
     }
     else{
         return redirect()->route('user-dashboard')->with('success','Please Purchase Packages');
     }
    }

     public  function bannercreate(Request $request, UserSubscription $UserSubscription)
    {
        ...
        $subscription = UserSubscription::find($input['id']);
        if ($file = $request->file('image')) 
         {      
            $name = time().$file->getClientOriginalName();
            $file->move('uploads/banners',$name);           
            $input['main_banner_iamge'] = $name;
        } 
         $subscription->update();
        //--- Logic Section Ends

        //--- Redirect Section        
       return redirect()->route('user-dashboard')->with('success','Banner Added  Successfully');    
