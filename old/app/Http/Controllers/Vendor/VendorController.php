...
class VendorController extends Controller
{
        ...
        public function banner_uploads()
    {
       $sub = UserSubscription::orderBy('id', 'DESC')->where('transaction_status','=',1)->where('subscription_type','=',2)->where('user_id','=',Auth::user()->id)->limit(1)->get(); 
        ...
        $sub = $sub['0'];
        $data = Auth::user(); 
        return view('vendor.purchase_banner',compact('sub','data'));
    }
     public function banner_update(Request $request, UserSubscription $UserSubscription)
    {
        //--- Validation Section
        $rules = [
               'shop_image'  => 'mimes:jpeg,jpg,png,svg',
       ...
       $subscription = UserSubscription::find($input['id']);  
        ...
        if ($file = $request->file('shop_image')) 
         {      
            ...
            if($subscription->banner_type == 1 )       
            $input['main_banner_1'] = $name;
           if($subscription->banner_type == 2 )
            $input['main_banner_2'] = $name;
           if($subscription->banner_type == 3 )
            $input['side_banner_1'] = $name;
           if($subscription->banner_type == 4 )
            $input['side_banner_2'] = $name;
        }

        $subscription->update($input);
        $msg = 'Successfully updated your banner';
        return response()->json($msg); 
    }
